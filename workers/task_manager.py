...
class TaskManager:
    ...
    def __init__(self):
        ...
        self.prediction_queue: Optional[Queue] = None
        self.retraining_queue: Optional[Queue] = None  # Новая очередь для переобучения
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.prediction_queue = Queue('predictions', connection=self.redis_conn, default_timeout=600)  # 10 минут
            
            # Создаем очередь RQ для переобучения
            logger.debug("TaskManager: Создание очереди 'retraining'...")
            self.retraining_queue = Queue('retraining', connection=self.redis_conn, default_timeout=7200)  # 2 часа
            ...
            logger.error(f"❌ Неизвестная ошибка при инициализации TaskManager: {e}", exc_info=True)
            self.redis_conn = None
            self.prediction_queue = None
            self.retraining_queue = None

    def enqueue_prediction(self, chat_id: int, home_team: str, away_team: str, job_id: str, priority: str = 'normal') -> Optional[Job]:
        ...
        """
        try:
            logger.debug(f"[{job_id}] enqueue_prediction: Начало постановки задачи в очередь для {home_team} vs {away_team} с приоритетом {priority}")
            ...
            if not self.prediction_queue:
                logger.error(f"[{job_id}] enqueue_prediction: Очередь prediction_queue не инициализирована!")
                return None
            logger.debug(f"[{job_id}] enqueue_prediction: Redis и очередь инициализированы корректно.")
            # - КОНЕЦ ДИАГНОСТИКИ -
            ...
            logger.error(f"[{job_id}] ❌ Ошибка при постановке задачи прогнозирования в очередь: {e}", exc_info=True)
            return None

    def enqueue_retraining(self, reason: str = "scheduled", season_id: Optional[int] = None) -> Optional[Job]:
        ...
        """
        try:
            logger.debug(f"enqueue_retraining: Начало постановки задачи переобучения (причина: {reason})")
            # - ДИАГНОСТИКА: Проверка инициализации -
            if not self.redis_conn:
            ...
            job_obj = self.retraining_queue.enqueue(
                ...
                ttl=86400,  # Время жизни задачи в очереди 1 день
                result_ttl=86400,  # Время жизни результата 1 день
                meta={'reason': reason, 'type': 'retraining', 'timestamp': datetime.now().isoformat()}
            )
            logger.info(f"✅ Задача переобучения успешно поставлена в очередь (причина: {reason})")
            return job_obj
        except Exception as e:
            logger.error(f"❌ Ошибка при постановке задачи переобучения в очередь: {e}", exc_info=True)
            return None
    ...
    def check_and_trigger_retraining(self, new_matches_count: int = 0, force_check: bool = False) -> bool:
        """
        ...
        try:
            ...
            retrain_model_age_threshold_days = getattr(settings, 'RETRAIN_MODEL_AGE_THRESHOLD_DAYS', 7)
            
            logger.debug(f"Проверка условий переобучения: "
            ...
            if new_matches_count >= retrain_new_matches_threshold:
                ...
                logger.info(f"Триггер переобучения: накопилось {new_matches_count} новых матчей "
                    ...
                    if poisson_regression_model.is_model_outdated():
                        ...
                        if last_updated_str:
                            ...
                            if model_age_days >= retrain_model_age_threshold_days:
                                ...
                                logger.info(f"Триггер переобучения: модель устарела на {model_age_days} дней "
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            retrain_finished_registry = FinishedJobRegistry('retraining', connection=self.redis_conn)
    ...
    def get_failed_jobs(self, queue_name: str = 'predictions', limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for job_id in registry.get_job_ids()[:limit]:
                try:
                    ...
                    logger.error(f"Ошибка при получении данных проваленной задачи {job_id}: {job_error}")
                    continue
            return failed_jobs
        except Exception as e:
...
def enqueue_prediction(chat_id: int, home_team: str, away_team: str, job_id: str, priority: str = 'normal') -> Optional[Job]:
...
def main():
    ...
    parser.add_argument('action', choices=['stats', 'cleanup', 'failed'], help='Действие для выполнения')
    parser.add_argument('--days', type=int, default=7, help='Количество дней для хранения задач (для cleanup)')
