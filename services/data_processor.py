...
class DataProcessor:
    ...
    def compute_rest_days(
        ...
        try:
            ...
            logger.debug(
                f"Вычислено дней отдыха: {rest_days} (с {prev_match_dt} по {next_match_dt})"
            )
            return rest_days
        except Exception as e:
    ...
    def compute_travel_load(
        ...
        try:
            ...
            for a, b in zip(recent_fixtures, recent_fixtures[1:], strict=False):
                ...
                if lat1 is None or lon1 is None or lat2 is None or lon2 is None:
                    logger.warning(
                        "Одна из координат для расчета расстояния равна None, используется 0"
                    )
                    lat1 = lat1 or 0
                    lon1 = lon1 or 0
                    lat2 = lat2 or 0
    ...
    def calculate_rolling_intensity(
        ...
        try:
            ...
            logger.debug(
                f"Рассчитана скользящая интенсивность: {intensity:.2f} за {window_days} дней"
            )
            return intensity
        except Exception as e:
    ...
    async def _calculate_team_fatigue(
        ...
        try:
            ...
            tasks = [
                asyncio.create_task(
                    self.client.get_last_team_matches(home_team_id, date_from=date_from)
                ),
                asyncio.create_task(
                    self.client.get_last_team_matches(away_team_id, date_from=date_from)
                ),
            ]
            home_fixtures_raw, away_fixtures_raw = await asyncio.gather(
                *tasks, return_exceptions=True
            )
            # Обработка возможных исключений
            if isinstance(home_fixtures_raw, Exception):
                logger.error(
                    f"Ошибка при получении матчей для команды {home_team_id}: {home_fixtures_raw}"
                )
                home_fixtures_raw = []
            if isinstance(away_fixtures_raw, Exception):
                logger.error(
                    f"Ошибка при получении матчей для команды {away_team_id}: {away_fixtures_raw}"
                )
                away_fixtures_raw = []
            ...
            away_rest_days = 0
            if home_fixtures:
                last_home_match_dt = datetime.fromisoformat(
                    home_fixtures[-1]["date"].replace("Z", "+00:00")
                )
                home_rest_days = self.compute_rest_days(last_home_match_dt, match_date)
            if away_fixtures:
                last_away_match_dt = datetime.fromisoformat(
                    away_fixtures[-1]["date"].replace("Z", "+00:00")
                )
            ...
            away_player_minutes = self.aggregate_minutes(
            ...
            logger.debug(
                f"Усталость: {home_team_id} {home_fatigue_data}, {away_team_id} {away_fatigue_data}"
            )
            return home_fatigue_data, away_fatigue_data
        except Exception as e:
            logger.error(
                f"Ошибка при расчете усталости команд {home_team_id} и {away_team_id}: {e}"
            )
            return {
                "rest_days": 0,
                "rest_days_flag": False,
                "travel_load": {},
                "rolling_intensity": 0.0,
    ...
    def compute_match_importance(
        ...
        try:
            ...
            logger.debug(
                f"Вычислена важность матча: {imp:.3f} (dist_releg={dist_releg}, dist_euro={dist_euro}, rounds_left={rounds_left})"
            )
            return imp
        except Exception as e:
    ...
    def core_availability(self, last5_lineups: list[dict[str, Any]]) -> float:
        ...
        try:
            ...
            logger.debug(
                f"Доступность ядра: {availability:.3f} (доступно {available}/{total} минут ключевых игроков)"
            )
            return availability
        except Exception as e:
    ...
    def ewma(
        ...
        try:
            ...
            for d in dates[::-1]:  # Перебираем даты в обратном порядке
                ...
                w = 0.5 ** (
                    delta / max(half_life_days, 1e-6)
                )  # Защита от деления на 0 или очень маленькое число
                weights.append(w)
            ...
            logger.debug(
                f"Рассчитано EWMA: {result:.4f} (период полураспада: {half_life_days} дней)"
            )
            return result
        except Exception as e:
    ...
    def opponent_adjust(self, value: float, opp_def_rating: float) -> float:
        ...
        try:
            ...
            adjusted_value = (value or 0.0) - (
                (opp_def_rating or 0.0) - 0.0
            )  # 0.0 - средний соперник
            logger.debug(
                f"Корректировка значения {value:.4f} с учетом силы соперника ({opp_def_rating}): {adjusted_value:.4f}"
            )
    ...
    def strength_adjusted_metric(self, raw: float, opp_strength: float) -> float:
        ...
        try:
            ...
            logger.debug(
                f"Корректировка метрики {raw:.4f} с учетом силы соперника ({opp_strength}): {adjusted_value:.4f}"
            )
            return adjusted_value
        except Exception as e:
            logger.error(
    ...
    def impute_weather(self, venue_city: str, match_month: int) -> dict[str, Any]:
        ...
        try:
            ...
            if not city_norms:
                ...
                logger.warning(
                    f"Не найдены климатические нормы для города {venue_city}, используем средние значения"
                )
                climate_data = {"temp_c": 15, "wind_mps": 3.0, "rain_prob": 0.25}
            else:
                # Получаем нормы для месяца
                climate_data = city_norms.get(
                    match_month or 0, {"temp_c": 15, "wind_mps": 3.0, "rain_prob": 0.25}
                )
            # Добавляем флаг импутации
            climate_data["is_imputed"] = True
            logger.debug(
                f"Импутированы погодные данные для {venue_city}, месяц {match_month}: {climate_data}"
            )
            return climate_data
        except Exception as e:
    ...
    def travel_features(
        self, team_venue: dict[str, Any], match_venue: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            result = {"km_trip": km, "tz_shift": tz_shift, "red_eye": red_eye}
    ...
    def streak_features(
        self, last_n_results: list[dict[str, Any]], team_season_avg: float = 1.5
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            last_3_matches = (
                (last_n_results or [])[-3:]
                if len(last_n_results or []) >= 3
                else (last_n_results or [])
            )
            ...
            result = {"win_streak": win_streak, "dry_spell": dry_spell, "burst": burst}
    ...
    def add_missing_mask(
        ...
        try:
            ...
            logger.debug(
                f"Добавлена маска пропусков: {missing_count}/{total_features} пропущенных значений, "
    ...
    async def get_match_context(self, fixture_id: int) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            if standings and rounds_left is not None:
                ...
                home_table_row = next(
                    (s for s in standings if s.get("team_id") == home_team_id), None
                )
                away_table_row = next(
                    (s for s in standings if s.get("team_id") == away_team_id), None
                )
                if home_table_row and away_table_row:
                    ...
                    logger.warning(
                        f"Не удалось найти строки таблицы для команд {home_team_id} и {away_team_id}"
                    )
            else:
    ...
    async def process_match(
        ...
        try:
            ...
            home_stats_task = asyncio.create_task(
                self.client.get_team_stats(home_team_id, match_date)
            )
            away_stats_task = asyncio.create_task(
                self.client.get_team_stats(away_team_id, match_date)
            )
            # Добавлено: Получаем PPDA домашней и гостевой команд
            home_ppda_task = asyncio.create_task(
                self.client.get_team_stats(home_team_id, match_date, stat_type="ppda")
            )
            away_ppda_task = asyncio.create_task(
                self.client.get_team_stats(away_team_id, match_date, stat_type="ppda")
            )
            ...
            if isinstance(home_stats, Exception):
                logger.error(
                    f"Ошибка при получении статистики команды {home_team_id}: {home_stats}"
                )
                home_stats = {}
            if isinstance(away_stats, Exception):
                logger.error(
                    f"Ошибка при получении статистики команды {away_team_id}: {away_stats}"
                )
                away_stats = {}
            ...
            if isinstance(home_build_up, Exception):
                logger.error(
                    f"Ошибка при получении Build-up Play команды {home_team_id}: {home_build_up}"
                )
            else:
            ...
            if isinstance(away_build_up, Exception):
                logger.error(
                    f"Ошибка при получении Build-up Play команды {away_team_id}: {away_build_up}"
                )
            else:
            ...
            try:
                if isinstance(result, Exception):
                    ...
                    processed_results.append(
                        {"match": matches[i], "success": False, "error": str(result)}
                    )
                else:
                ...
                logger.error(
                    f"Ошибка при обработке результата для матча {matches[i]}: {process_error}"
                )
                processed_results.append(
                    {"match": matches[i], "success": False, "error": str(process_error)}
                )
...
def compute_time_decay_weights(
    df: pd.DataFrame, *, date_col: str, half_life_days: int
) -> np.ndarray:
