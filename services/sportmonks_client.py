...
from database.cache_postgres import cache, versioned_key, set_with_ttl # Импортируем необходимые функции
...
async def fetch_lineup_api(fixture_id: int) -> Optional[Dict[str, Any]]:
...
class SportMonksClient:
    ...
    async def get_fixture(self, fixture_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            params = {"api_token": self.api_token, "include": "localTeam,visitorTeam,venue"}
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    data = await response.json()
                    return data.get("data")
                else:
                    logger.error(f"Ошибка получения данных матча {fixture_id}: статус {response.status}")
                    return None
        except Exception as e:
            logger.error(f"Ошибка при получении данных матча {fixture_id}: {e}", exc_info=True)
            return None
    @cached(ttl=900)  # Кэшируем на 15 минут
    async def get_weather(self, team_id: int, match_date: date) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Прогноз погоды для команды {team_id} на {match_date} недоступен: статус {response.status}")
                    return None
        except Exception as e:
            logger.error(f"Ошибка при получении прогноза погоды для команды {team_id}: {e}", exc_info=True)
            return None
    # --- Изменено: Использование специфичного TTL и инвалидации для лайнапов ---
    # @cached(ttl=120)  # Кэшируем на 2 минуты - Удалено
    async def get_lineups(self, fixture_id: int) -> Optional[Dict[str, Any]]:
        ...
        if cache:
            try:
                ...
                if cached_lineups is not None:
                     logger.debug(f"Составы для матча {fixture_id} получены из кэша (специальный TTL)")
                ...
                if response.status == 200:
                    ...
                    if cache and lineups_data is not None:
                         try:
                             ...
                             await set_with_ttl(cache.redis_client, key, lineups_data, "lineups_fast")
                             logger.debug(f"Составы для матча {fixture_id} сохранены в кэш с TTL=lineups_fast")
                         except Exception as e:
                             logger.error(f"Ошибка при сохранении составов в кэш для матча {fixture_id}: {e}")
                    return lineups_data
                else:
                    logger.warning(f"Составы для матча {fixture_id} недоступны: статус {response.status}")
                    return None
        except Exception as e:
            logger.error(f"Ошибка при получении составов для матча {fixture_id}: {e}", exc_info=True)
            return None
    # --- Конец изменения ---
    @cached(ttl=120)  # Кэшируем на 2 минуты
    async def get_injuries(self, team_id: int) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    injuries = [player for player in squad_data if player.get("player", {}).get("injured")]
                    return injuries
                else:
                    logger.warning(f"Информация о травмах команды {team_id} недоступна: статус {response.status}")
                    return None
        except Exception as e:
            logger.error(f"Ошибка при получении информации о травмах команды {team_id}: {e}", exc_info=True)
            return None
    @cached(ttl=3600)  # Кэшируем на 1 час
    async def get_table(self, fixture_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Ошибка получения таблицы для матча {fixture_id}: статус {response.status}")
                    return None
        except Exception as e:
            logger.error(f"Ошибка при получении таблицы для матча {fixture_id}: {e}", exc_info=True)
            return None
    @cached(ttl=3600)  # Кэшируем на 1 час
    async def get_last_team_matches(self, team_id: int, limit: int = 10, date_from: Optional[str] = None,
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Ошибка получения последних матчей команды {team_id}: статус {response.status}")
                    return []
        except Exception as e:
            logger.error(f"Ошибка при получении последних матчей команды {team_id}: {e}", exc_info=True)
            return []
    @cached(ttl=3600)  # Кэшируем на 1 час
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Ошибка получения статистики команды ID {team_id}: статус {response.status}")
            ...
            logger.error(f"Ошибка при обработке статистики команды ID {team_id}: {e}", exc_info=True)
            # Возвращаем минимальную статистику в случае ошибки
            return {
                "team_id": team_id,
                "team_name": "Unknown",
    ...
    async def get_upcoming_fixture(self, home_team_id: int, away_team_id: int) -> Optional[Dict[str, Any]]:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    if fixtures:
                        ...
                        logger.warning(f"Матч между командами {home_team_id} и {away_team_id} не найден")
                        return None
                else:
                    logger.error(f"Ошибка поиска матча между {home_team_id} и {away_team_id}: статус {response.status}")
                    return None
        except Exception as e:
            logger.error(f"Ошибка при поиске матча между {home_team_id} и {away_team_id}: {e}", exc_info=True)
            return None
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    data = await response.json()
                    fixtures = data.get("data", [])
                    logger.debug(f"Получено {len(fixtures)} матчей для лиги {league_id}, сезона {season_id}")
                    return fixtures
                else:
                    logger.error(f"Ошибка получения матчей для лиги {league_id}, сезона {season_id}: статус {response.status}")
                    return []
        except Exception as e:
            logger.error(f"Ошибка при получении матчей для лиги {league_id}, сезона {season_id}: {e}", exc_info=True)
            return []
