...
class BivariatePoisson:
    ...
    def __init__(self, lam1: float, lam2: float, rho: float):
        ...
        logger.debug(f"Bivariate Poisson инициализирован: λ1={self.l1:.3f}, λ2={self.l2:.3f}, ρ={self.rho:.3f}")
    def _log_prob(self, x: int, y: int) -> float:
        ...
        try:
            ...
            if self.rho == 0:
                log_p = (
                    x * np.log(self.l1) - self.l1 - gammaln(x + 1) +
                    y * np.log(self.l2) - self.l2 - gammaln(y + 1)
                )
                return log_p
            ...
            if max_k == 0:
                ...
                log_p = (
                    x * np.log(self.l1 - self.rho) - (self.l1 - self.rho) - gammaln(x + 1) +
                    y * np.log(self.l2 - self.rho) - (self.l2 - self.rho) - gammaln(y + 1) +
                    -self.rho
                )
                return log_p
            ...
            for k in range(max_k + 1):
                log_prob_k = (
                    k * np.log(self.rho) - self.rho - gammaln(k + 1) +
                    (x - k) * np.log(self.l1 - self.rho) - (self.l1 - self.rho) - gammaln(x - k + 1) +
                    (y - k) * np.log(self.l2 - self.rho) - (self.l2 - self.rho) - gammaln(y - k + 1)
                )
...
def estimate_rho(feature_vector: Dict[str, Any], default_rho: float = 0.1) -> float:
    ...
    try:
        ...
        rho_estimate = (
            0.05 + 
            0.08 * style_mismatch +  # Влияние стилевого несоответствия
            0.03 * (1 - match_importance) +  # Влияние важности матча
            0.04 * fatigue_intensity  # Влияние усталости
        )
...
def score_matrix(lam_home: float, lam_away: float, rho: float, max_goals: int = 10, apply_dixon_coles: bool = True) -> np.ndarray:
...
def outcome_probabilities(lam_home: float, lam_away: float, rho: float, max_goals: int = 10, apply_dixon_coles: bool = True) -> Dict[str,float]:
    M = score_matrix(lam_home, lam_away, rho, max_goals=max_goals, apply_dixon_coles=apply_dixon_coles)
    p_home = float(np.triu(M, k=1).sum())
    p_away = float(np.tril(M, k=-1).sum())
    p_draw = float(np.trace(M))
    p_btts_yes = float(M[1:,1:].sum()); p_btts_no = 1.0 - p_btts_yes
    p_over_2_5 = float(sum(M[gh,ga] for gh in range(M.shape[0]) for ga in range(M.shape[1]) if (gh+ga)>2))
    p_under_2_5 = 1.0 - p_over_2_5
