...
class CVResult:
    ...
    best_model: Any
def save_artifacts(model, path: str, meta: Optional[dict]=None) -> None:
...
class PoissonRegressionModel:
    ...
    def __init__(self, alpha: float = 0.001, max_iter: int = 300):
        ...
        self.team_ratings = {}
        logger.info(f"Инициализация Poisson-регрессионной модели (sklearn). Alpha: {alpha}, Max_iter: {max_iter}")
    ...
    def load_ratings(self, filepath: str = "data/team_ratings.json"):
        ...
        try:
            if os.path.exists(filepath):
                ...
                logger.warning(f"Файл с рейтингами {filepath} не найден. team_ratings останется пустым.")
                self.team_ratings = {}
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str]]:
        """
        ...
        try:
            ...
            df_clean['home_league_interaction'] = df_clean['home_team_id_hash'] * df_clean['league_id_hash']
            df_clean['away_league_interaction'] = df_clean['away_team_id_hash'] * df_clean['league_id_hash']
            ...
            categorical_features = [
                'home_team_id_hash', 'away_team_id_hash', 'league_id_hash',
                'home_league_interaction', 'away_league_interaction'
            ]
            ...
            logger.info(f"Подготовлены признаки: {len(X_home)} домашних, {len(X_away)} гостевых записей.")
    ...
    def prepare_features_for_match(
        ...
        try:
            ...
            continuous_features_away = [
                away_rest_days, home_rest_days, away_km_trip, home_km_trip, # Переставлены rest_days и km_trip
                away_xg, home_xg, away_xga, home_xga, # Переставлены xg, xga
                away_ppda, home_ppda, away_oppda, home_oppda, # Переставлены ppda, oppda
                away_mismatch, home_mismatch, # Переставлен mismatch
                away_league_zscore_attack, home_league_zscore_attack, # Переставлены zscore_attack
                away_league_zscore_defense, home_league_zscore_defense # Переставлены zscore_defense
            ]
            categorical_features_away = [
                away_team_id_hash, home_team_id_hash, league_id_hash, # Переставлены team hashes
                away_league_interaction, home_league_interaction # Переставлены interactions
            ]
            X_home_single = np.array(continuous_features_home + categorical_features_home).reshape(1, -1)
            X_away_single = np.array(continuous_features_away + categorical_features_away).reshape(1, -1)
            ...
            feature_count = len(self.feature_names) if self.feature_names else 23 # Примерное количество признаков
            ...
            for league_id in matches_df['league_id'].unique():
                ...
                if len(league_data) >= 10:  # Минимум 10 матчей для расчета
                    ...
                    for _, row in league_data.iterrows():
                        ...
                        home_avg = league_data[league_data['home_team_id'] == home_team]['home_goals'].mean()
                        away_avg = league_data[league_data['away_team_id'] == away_team]['away_goals'].mean()
                    ...
                    if lambda_values:
                        ...
                        logger.debug(f"Лига {league_id}: p99 = {p99:.3f} (на основе {len(lambda_values)} значений)")
                else:
    ...
    def dynamic_cap(self, lam: float, p99: float) -> float:
        ...
        try:
            ...
            if lam > p99:
                logger.debug(f"Ограничено значение λ: {lam:.3f} -> {capped_lambda:.3f} (p99={p99:.3f})")
            return capped_lambda
        except Exception as e:
            logger.error(f"Ошибка при применении динамического ограничения: {e}")
            return lam

    async def train_model(self, training_data: pd.DataFrame) -> Optional[bool]:
    ...
    def calculate_base_lambda(
        ...
        try:
            ...
            if self.scaler is None or self.model_home is None or self.model_away is None or not self.feature_names:
                logger.warning("Модель не обучена или отсутствуют компоненты. Используются усредненные значения.")
