...
- **Шаги выполнения**:
  - [x] Убедился в наличии пакета `telegram` с инициализационным модулем для импорта.
  - [x] Перевёл импорты middleware и обработчиков в `telegram/bot.py` на относительный синтаксис.
  - [x] Провёл smoke-проверки импортов и зафиксировал изменения в документации.
- **Зависимости**: telegram/bot.py, docs/changelog.md, docs/tasktracker.md
...
- **Шаги выполнения**:
  - [x] Добавлен GitHub Actions джоб `style-and-tests-pr` с выборочным форматированием/проверкой Python-файлов и smoke/pytest.
  - [x] Сохранён полнообъёмный пуш-джоб `style-and-tests-main` для автопочинки и проверок всего репозитория.
  - [x] Обновлена конфигурация Ruff до профиля `E`,`F`,`I` и расширены исключения легаси-директорий.
  - [x] Переписана цель `make check`, чтобы линтеры запускались только на `origin/main...HEAD` изменениях перед `pytest -q`.
- **Зависимости**: .github/workflows/ci.yml, pyproject.toml, Makefile, docs/changelog.md, docs/tasktracker.md
...
- **Шаги выполнения**:
  - [x] Перестроены импорты и загрузка модулей в `tests/conftest.py` до исполняемого кода с использованием `import_module`.
  - [x] Добавлен пакет `tests/_stubs/alembic` и принудительная регистрация заглушек в `sys.modules` при отсутствии `command`.
  - [x] Запущены проверки Ruff/Black/isort и `make check` (pytest падает на существующих функциональных ошибках). 
- **Зависимости**: tests/conftest.py, tests/_stubs/alembic/*, docs/changelog.md, docs/tasktracker.md
...
- **Шаги выполнения**:
  - [x] Создан документ `reports/FINAL_SUMMARY.md` с перекрёстными ссылками на аудит, инвентаризацию, run summary и release notes.
  - [x] Обновлены `docs/changelog.md` и `docs/tasktracker.md` для фиксации итогового шага.
  - [x] Проверена связность ссылок и оформление согласно `.codexrules`.
- **Зависимости**: reports/FINAL_SUMMARY.md, docs/changelog.md, docs/tasktracker.md
...
- **Шаги выполнения**:
  - [x] Реализован сбор статистики из SQLite с обработкой отсутствующих таблиц и режимом DEGRADED.
  - [x] Переписан обработчик `/start` для повторного использования клавиатур и отображения версии/метрик.
  - [x] Добавлены тесты, проверяющие корректность сообщения статистики и поведение без БД.
- **Зависимости**: telegram/handlers/start.py, tests/bot/test_handlers_smoke.py, docs/changelog.md, tickets/TODO.md, docs/tasktracker.md
