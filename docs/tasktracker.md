...
- **Шаги выполнения**:
  ...
  - [x] Экспорт ECE/LogLoss после каждого завершённого матча
  - [x] Публикация текущих ECE/LogLoss при каждом новом прогнозе
  - [x] Нулевые значения метрик при отсутствии завершённых матчей
